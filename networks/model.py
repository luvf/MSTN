...
def update_centers(model, s_gen, t_gen, s_true, t_clf, args):
	...
	s_center = torch.zeros(model.n_class, model.n_features).to(device=args.device)
	t_center = torch.zeros(model.n_class, model.n_features).to(device=args.device)
...
def fit(args, epochs, model, opt, dataset, eval_func, valid):
	out = list()
	for epoch in range(epochs):
		model.train()
		for sx, sy, tx,_ in dataset:
			s_loss, c_loss, G_loss, D_loss = loss_batch(model, sx, tx, sy, opt, args)
			#print("aaa")
			#print(model)
		print("epoch: {} --- s_loss: {}, c_loss: {}, G_loss: {}, D_loss: {}".format(epoch, s_loss, c_loss, G_loss, D_loss))
		model.eval()
		with torch.no_grad():
			loss = np.zeros(5)
			for sx, sy, tx, ty in valid:
				loss += eval_batch(model, sx, tx, sy, ty)
			loss /= len(valid)-1
			print(epoch, loss)
